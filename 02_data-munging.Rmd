...
# Create parks and years arguments
parks <- c("ROMO","ACAD","LAKE","YELL","GRCA","ZION","OLYM","GRSM")

years <- (1980:2021)

# Map this stuff to iteratively pull information from National Park Service API

map_visitation <- parks %>%
  map(~unit_visitation(.,start_year = 1980, end_year = 2021))


units <- bind_rows(map_visitation)
...
# I learned how to do this from others
plotly::ggplotly(
  ggplot(data = yearly) + 
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
...
data1 <- wide_data %>%
  pivot_longer(cols = -c(Year, Parkwide),
               names_to = "UnitCode",
               values_to = "RecVisitation") %>%
  mutate(., Percentage = (RecVisitation/Parkwide)*100)

plotly::ggplotly(
  ggplot(data = data1) + 
    geom_col(aes(x = Year, y = Percentage, color = UnitCode)) +
...
```{r}
#-c(Year, Parkwide) indicates the columns that I don't want to gather into columns
# worked on this with Meggi
data2 <- joined_data %>%
  group_by(Year) %>%
  mutate(Percentage = (RecreationVisitors.x/RecreationVisitors.y)*10)


plotly::ggplotly(
  ggplot(data = data2) + 
    geom_col(aes(x = Year, y = Percentage, color = UnitCode.x)) +
    scale_y_continuous(labels = scales::label_number()) +
    theme_bw(base_size=10)
    )
```
...
data3 <- wide_data %>%
  ...
  mutate(Parkwide = NULL) %>%
  mutate_at(.vars = parks, mean) %>%
  pivot_longer(cols = -c(Year),
              names_to = "UnitCode",
              values_to = "RecVisitation") %>%
  filter(Year == 1990) %>%
  mutate(rank = rank(desc(RecVisitation)))


```
Answer: GRSM has the most visitation. ACAD has the least visitation.
